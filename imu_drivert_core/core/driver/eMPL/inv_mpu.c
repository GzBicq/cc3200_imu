...
void inv_mpu_close(void)
{
  unsigned char data;
  
  data=0x3f;
  Sensors_I2C_WriteRegister(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data);
 
  data=0x47;//47
  Sensors_I2C_WriteRegister(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data);
  
}

void inv_mpu_user_motion_interrupt_mode(unsigned short thresh, unsigned short lpa_freq)
{
#ifdef MPU6500
    unsigned char data[3];
    unsigned char thresh_hw;
